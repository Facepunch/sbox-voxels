...
namespace Facepunch.Voxels
{
	...
	public partial class Chunk : IValid
	{
		...
		public bool NeedsMeshCollisionRebuild { get; private set; }
		...
		private List<int> CollisionIndices{ get; set; } = new();
		...
		public async void FullUpdate()
		{
			...
			try
			{
				...
				NeedsMeshCollisionRebuild = true;
		...
		public void BuildMesh()
		{
			...
			try
			{
				for ( int i = 0; i < RenderLayers.Count; i++ )
				{
					...
					if ( vertices.Count > 0 )
					{
						layer.Mesh.SetVertexBufferData( vertices, vertexCount );
		...
		private void ServerTick()
		{
			...
			if ( NeedsMeshCollisionRebuild && !IsFullUpdateTaskRunning() )
			{
				NeedsMeshCollisionRebuild = false;
				BuildCollision();
			}
		}
		...
		private void ClientTick()
		{
			...
			if ( !AreAdjacentChunksUpdating() )
			{
				if ( NeedsMeshCollisionRebuild )
				{
					NeedsMeshCollisionRebuild = false;
